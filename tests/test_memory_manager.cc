...
void zeroed_at_initialization() {
    ...
    // Check validity.
    void* const mem_ptr = memory_manager.allocator.memory;
...
void initialization_and_shutdown() {
    ...
    for (usize idx = 0; idx < block_length; ++idx) {
        u64 const        expected = idx * start_value;
        u64 const* const actual   = reinterpret_cast<u64 const*>(
...
void memory_statistics() {
    ...
    usize const g_size = wrap_sub(memory_manager.allocator.offset, used);
    ...
    usize const actual_used_memory = memory_manager.allocator.offset;
