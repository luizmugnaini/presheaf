...
void test_string_view() {
    ...
    constexpr StringView v2 = psh_string_view("Nine for the Elven-kings under moon and star");
    psh_assert(str_equal(v2.data.buf, "Nine for the Elven-kings under moon and star"));
    psh_assert(v2.data.size == std::strlen("Nine for the Elven-kings under moon and star"));
    psh_assert(v2.data.buf[v1.data.size] == 0);

    StringView v3{"Nine for the Elven-kings under moon and star"};
    psh_assert(str_equal(v3.data.buf, "Nine for the Elven-kings under moon and star"));
...
void test_string() {
    Arena arena{reinterpret_cast<u8*>(std::malloc(512)), 512};
    {
        ...
        psh_assert(str_equal(s.data.buf, "Seven for the Dwarf-lords in their halls of stone"));
...
void test_string_join() {
    constexpr Str check_str1 = psh_str("One ring to rule them all, "
                                       "One ring to find them, "
                                       "One ring to bring them all, "
                                       "and in the darkness bind them.");

    constexpr Buffer<StringView const, 4> views1{
    ...
    constexpr Str check_str2 = psh_str("Three Rings for the Elven-kings under the sky,\n"
    ...
    Arena arena{reinterpret_cast<u8*>(std::malloc(psh_kibibytes(5))), psh_kibibytes(5)};
    {
        // Empty string.
